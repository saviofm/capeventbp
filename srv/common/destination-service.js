...
async function getDestinationUrlWorkflow(service) {
  return service.credentials.endpoints.api;
}
...
async function serviceCall(name, path, method, headers, body) {
  ...
  const jwtToken = await getJWTToken('capeventbp-xsuaa-service');
  if (!headers) {
    headers = new fetch.Headers({
      'Content-Type': 'application/json'
    });
...
async function getDestinationUrl(name) {
  const destinationServiceName = 'capeventbp-destination-service'
